...
void CostComputor::ComputeGradient() const
{
	const sint32 width = width_;
	const sint32 height = height_;
	if (width <= 0 || height <= 0 ||
		grad_left_ == nullptr || grad_right_ == nullptr ||
		gray_left_.empty() || gray_right_.empty())
	{
		return;
	}
...
void CostComputor::ComputeCost()
{
	...
	for (sint32 y = 0; y < height_; y++)
	{
		for (sint32 x = 0; x < width_; x++)
		{
			...
			const auto gradLx = (grad_left_[y * width_ + x].x) * gray_left_[y * width_ + x];
			const auto gradLy = (grad_left_[y * width_ + x].y) * gray_left_[y * width_ + x];
			// 逐视差计算代价值
			for (sint32 d = min_disparity_; d < max_disparity_; d++)
			{
				...
				// 梯度代价
				auto gradRx = (grad_right_[y * width_ + x].x) * gray_right_[y * width_ + xr];
				auto gradRy = (grad_right_[y * width_ + x].y) * gray_right_[y * width_ + xr];
				const float32 cost_grad = abs(gradLx - gradRx) + abs(gradLy - gradRy);
				...
				// ad-census代价
				cost = 1 - exp(-cost_ad / lambda_ad) + 1 - exp(-cost_census / lambda_census) + 1 - exp(-cost_grad);
			}
		}
	}
}
