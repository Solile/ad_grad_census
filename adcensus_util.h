...
{
	void census_transform_5x5(const uint8 *source, vector<uint32> &census, const sint32 &width, const sint32 &height);
	/**
	* \brief census变换
	...
	* \param height	输入，影像高
	*/
	void census_transform_9x7_improved(const uint8* source, vector<uint64>& census, const sint32& width, const sint32& height);

	void census_transform_dx2d(const uint8 *source, vector<uint64>& census, const sint32 &width, const sint32 &height);
